...
-- Enforce that profiles are NOT created until email is verified!
-- Run this in your Supabase SQL Editor:
-- (Supabase Dashboard > SQL Editor > New Query > Run)
-- ==========================================================

-- 1. Update the trigger function to return early if email is not confirmed
...
DECLARE
  ...
  v_student_code text;
BEGIN
  -- 1a. Gating: Return early if the email is not verified yet
  IF new.email_confirmed_at IS NULL THEN
    RETURN new;
  END IF;

  -- 1b. Prevent duplicate profiles if the trigger runs multiple times
...
-- 2. Bind trigger function to AFTER INSERT OR UPDATE on auth.users
