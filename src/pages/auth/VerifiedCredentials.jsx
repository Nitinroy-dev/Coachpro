...
export default function VerifiedCredentials() {
  ...
  const [newlyVerifiedAdmin, setNewlyVerifiedAdmin] = useState(false)
      ...
      try {
        ...
        if (profile) {
          if (profile.temp_password) {
            ...
            // If they don't have a temp password, check if they are already verified in the database
        ...
        {newlyVerifiedAdmin ? (
          <div className="space-y-5">
            ...
            <p className="text-gray-500 text-xs leading-relaxed px-2">
              Your coaching institute account is now active and ready. You can log in using the credentials you created during signup.
            </p>
            <Button onClick={handleProceed} fullWidth className="mt-4" icon={ArrowRight}>
              Proceed to Log In
            </Button>
          </div>
        ) : alreadyProcessed ? (
